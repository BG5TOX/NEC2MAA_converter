...
- **hash 基线重审定**：i18n-4 批 723 文件全量重生成（CM 行 `by NEC2MAA v0.5.` + 告警英文）；v0.6 批因 CM 行升 v0.6 再次全量重生成——旧 v0.4 基线存档 `backups/preR21h_723_output_hashes.v04baseline.json`、旧 v0.5 基线 `backups/preR21h_723_output_hashes.v05baseline.json`，现行基线 `preR21h_723_output_hashes.json`（723 文件，含 20 库锥度 + jp2000）。
...
│   │   ├── zh.js       #   简体中文 (键集基准, 200 词条; values 与 v0.4 原句逐字一致)
│   │   └── en.js       #   English (键集与 zh 完全对齐)
│   ├── utils.js        # evalExpr/parseGsScale/segToDesignator/formatNum/hasChinese/removeChinese (告警→结构化条目)
│   ├── geometry.js     # collectWires (GW/GX/GM; GX packed I2、GM ITS(F7)+NRPT 累积、GW GC 锥度跳过; 告警→结构化条目)
...
│   │   └── maa-writer.js  # NEC 输出: CM 头(v0.6)/GW/EX 百分比锚+锥度锚点重映射(R21)/LD 表达式/GN/FR + ASCII 终检; R1 告警→LF() 英文
...
├── tools/              # 开发期 Node 脚本: bump_version.js (一键升版本: 改标记+重生成 723 基线+跑版本敏感测试)
├── backups/            # 批次 .bak 快照 (v04 R21/SF + v05 i18n1-4 + 修复轮 i18n5-10) + hash 基线×2 + README
└── docs/               # 格式文档×2 / 发布归档 v0.3–v0.6 / 开发历史日志 / i18n 方案 / 安全审计 / Ground.txt
```
...
- **GM 卡（2026-09-20 修复）**：NRPT>0 副本逐轮**累积变换**（C_j=T^j(原作)，NEC-2 `move()`），每轮 tag 只加一次 ITG（= 原作 + j·ITG）；**ITS (F7)** 只作用于「第一个 tag==ITS 的线段到结构末尾」（按线段位置，非 tag 数值比较；0/空=全结构；未命中→`n2m.gm.itsNotFound` 告警 + 按全结构从宽）；平移随 GS 缩放（与 GW 坐标同一系数）；预算按 ITS 后实际复制量 `n + nrpt·(n−start)`
- **GX 卡（2026-09-20 修复）**：I2 为**三位数字标志**（百/十/个位 = X/Y/Z 镜像；反射按字母逆序 Z→Y→X，tag 增量 1/2/4 倍 ITG；tag=0 为无效 tag 不递增）。**注意 `evalExpr` 前导 0 按八进制**（`new Function`：`"010"`→8），I2 必须按原始文本从右取位；逐位模板 `Math.floor(i2/100)%10` 不可用
- **GW 卡（2026-09-20 加固）**：半径空白或 0 = 后续 `GC` 锥度续行卡，MMANA 无等价物 → 跳过并按文件聚合单条 `n2m.gw.gcTaper` 告警（`{tag,count}`）；旧实现空白静默丢弃、0 产出非法零半径
...
- 2026-09-20（NEC 几何/卡片解析审计修复）：**GM ITS(F7)+NRPT 累积 + GX packed I2 + GW GC 锥度**（参照 `E:\coding_projects\HAM_Structural_Analysis` 的 `core/nec/geometry.ts` 与 NEC-2 手册）——
  ...
  - **GW**：半径空白或 0 = 后续 `GC` 锥度续行卡；MMANA 无锥度振子等价物 → 跳过并按文件聚合单条 `n2m.gw.gcTaper` 告警（`{tag,count}`）；旧实现空白静默丢弃、0 产出非法零半径
  - 语言包 zh/en 各 +2 词条（`n2m.gm.itsNotFound` / `n2m.gw.gcTaper`）；m3_geometry 增 14 断言（GM 累积/ITS、GX packed/多面/tag0、GW GC 跳过/聚合、样例 164 根），GS×GM 断言同步修正
  - **回归：17 套件 371 断言全绿**（m3 27/27）；备份 `*.gmfix.bak` / `*.cardfix.bak` / `*.v06.bak`
  - **v0.6 版本标记**：CM 行 `by NEC2MAA v0.6.` + index.html 徽标/页脚 + 语言包 footer 同步；**723 文件 hash 基线全量重审定**（旧 v0.5 基线存档 `backups/preR21h_723_output_hashes.v05baseline.json`）；r19/sf2/r21 版本断言同步 v0.6
  - **审计遗留（待用户裁决，未改）**：① **GS** 多卡仅取末条（未按 NEC 增量缩放）、GS 早于其引用 SY 时求值失败、<4 字段形式跳过；② **LD** 类型编号（项目 1=串联 RLC/2=并联 RLC/3=串 R+jX/4=并 G+jB/5=电导率）与 NEC-2 手册（0=串联/1=并联/2=串每米/3=并每米/4=阻抗/5=电导率）不同——现有 m4 断言按项目编号固化，需 4NEC2 实测裁决；③ **GN** 仅映射地面类型，εr/σ（F1/F2）与 NRADL/第二介质未读取（MMANA 侧以预设替代）
