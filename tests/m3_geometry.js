...
// 12. 单卡 NRPT=2 累积平移: 副本 = T^j(原作) → z=1,2 (旧实现两副本同位 z=1)
w = wiresOf('GW 1 1 0 0 0 1 0 0 0.01\nGM 0 2 0 0 0 0 0 1');
check('GM NRPT=2 累积平移: z=1,2', w.length === 3 && w[1].z1 === 1 && w[2].z1 === 2, JSON.stringify(w.map(x => x.z1)));
// 13. 单卡 NRPT=2 累积旋转: 每轮 90° → 90°/180° (旧实现两副本同为 90°)
w = wiresOf('GW 1 1 1 0 0 1 0 0 0.01\nGM 0 2 0 0 90 0 0 0');
check('GM NRPT=2 累积旋转: 90°/180°', w.length === 3 && Math.abs(w[1].y1 - 1) < 1e-9 && Math.abs(w[2].x1 + 1) < 1e-9, JSON.stringify(w.map(x => [x.x1, x.y1])));
// 14. ITS=F7 就地: 只变换"第一个 tag==ITS 的线段到末尾"(按位置), 0/空=全结构
w = wiresOf('GW 1 1 0 0 0 1 0 0 0.01\nGW 2 1 0 0 0 1 0 0 0.01\nGW 3 1 0 0 0 1 0 0 0.01\nGM 0 0 0 0 0 0 0 5 3');
check('GM ITS=3 就地: 仅 tag3 z+5', w.length === 3 && w[0].z1 === 0 && w[1].z1 === 0 && w[2].z1 === 5, JSON.stringify(w.map(x => x.z1)));
// 15. ITS 复制 (NRPT>0): 只复制 ITS 起后缀, 副本 tag 每轮只加一次 ITG
w = wiresOf('GW 1 1 0 0 0 1 0 0 0.01\nGW 2 1 0 0 0 1 0 0 0.01\nGW 3 1 0 0 0 1 0 0 0.01\nGM 10 1 0 0 0 0 0 5 3');
...
if (fs.existsSync(samplePath)) {
  ...
  check('样例 2M/70CM 堆叠 ITS: 164 根 (70cm 128 短 + 2m 36 长)',
        ...
        && shortGroup.filter(x => x.tag >= 1 && x.tag <= 16).length === 16
        && longGroup.filter(x => x.tag >= 101 && x.tag <= 109).length === 9,
        `n=${sw.length} short=${shortGroup.length} long=${longGroup.length}`);
} else {
  check('样例 2M/70CM 堆叠 ITS: 文件存在', false, samplePath);
}

// === 2026-09-20 卡片解析审计修复 (GX packed I2 + GW GC 锥度) ===
// 18. GX I2 十位=Y 镜像 (实际文件如 "GX 20 010"); 原实现把 010 当 X 标志
w = wiresOf('GW 7 1 1 2 3 1 2 3 0.01\nGX 10 010');
check('GX 010 → Y 镜像 (tag+10)', w.length === 2 && w[1].y1 === -2 && w[1].x1 === 1 && w[1].tag === 17, JSON.stringify(w.map(x => ({ t: x.tag, x: x.x1, y: x.y1 }))));
// 19. GX I2 个位=Z 镜像 ("GX n 001")
w = wiresOf('GW 7 1 1 2 3 1 2 3 0.01\nGX 10 001');
check('GX 001 → Z 镜像 (tag+10)', w.length === 2 && w[1].z1 === -3 && w[1].tag === 17, JSON.stringify(w[1]));
// 20. GX 011 = Y+Z 双面 → 4 根; Z 先反射 (+itg), Y 再反射 (+2·itg)
w = wiresOf('GW 7 1 1 2 3 1 2 3 0.01\nGX 1 011');
check('GX 011 → Y+Z 4 根 tags=[7,8,9,10]', w.length === 4 && JSON.stringify(w.map(x => x.tag)) === JSON.stringify([7, 8, 9, 10]), JSON.stringify(w.map(x => ({ t: x.tag, y: x.y1, z: x.z1 }))));
// 21. GX 111 = 三面 → 8 根
w = wiresOf('GW 7 1 1 2 3 1 2 3 0.01\nGX 1 111');
check('GX 111 → 三面 8 根', w.length === 8, 'n=' + w.length);
// 22. tag=0 为无效 tag, GX 镜像不递增 (与 GM 一致)
w = wiresOf('GW 0 1 1 2 3 1 2 3 0.01\nGX 5 100');
